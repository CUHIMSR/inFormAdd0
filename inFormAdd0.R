...
#library("phenoptr"); # for read_cell_seg_data(fname)
#library("flowCore"); # only necessary for read.FCS() and write.FCS()
...
make_fcs <- function(fname = "concat5_cell_seg_data.txt", outdir=".", dropNormal=FALSE){
   t0 <- Sys.time();
   ...
   if(dropNormal){
      ...
      if(length(grepNormal_i) > 0){
         print(sprintf("Dropping %i normals from %s", length(grepNormal_i), basename(fname)), q=F);
...
test_readback <- function(){
   ...
   fcsname <- "FCS_files/5.9.18_Merged_cell_Segmentation_Data_1_Lamina Propria_all.fcs";
   rb <- read.FCS(fcsname, transformation=c());
}
...
options(shiny.maxRequestSize=3000*1024^2); # 3GB for inputFile()

# Define UI ----
ui <- fluidPage(
  
  titlePanel("Add Zeros"),

  sidebarLayout(
    sidebarPanel(h2("inForm Absent Cell Category Correction"), h4("inForm omits categories with zero cells in a region of interest. Some workflows require all categories to be represented. Add zeros to ensure all regions and all samples have the same categories.")
  ),
...
# By default, Shiny limits file uploads to 5MB per file. You can modify this limit by using the shiny.maxRequestSize option. For example, adding options(shiny.maxRequestSize=30*1024^2) to the top of server.R would increase the limit to 30MB.
      ...
      br(),
      br(),
      h3('Please Note: This may take several minutes for large files.'),
...
server <- function(input, output) {
   ...
   #output$vtovalues <- renderText({ 
      #req(input$file);
      #addZeros_inform_merged(fname=input$file$datapath, origfname=input$file$name);
